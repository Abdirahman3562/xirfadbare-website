...
const sampleComments = [
  {
    content: "Maqaalkan aad buu ii caawiyay! Waxaan bilaabay barashada programming-ka waanad ku mahadsan tahay.",
    author: null, // Will be set to a user ID
  },
  {
    content: "Fadlan ma ii sheegi kartaa resource dheeraad ah oo aan ku barto JavaScript?",
    author: null, // Will be set to a user ID
  },
  {
    content: "Xirfadbare waa goob aad u fiican. Waxaan kuu soo jeedinayaa qof walba.",
    author: null, // Will be set to a user ID
  },
  {
    content: "Ma jiraan videos dheeraad ah oo ku saabsan React hooks?",
    author: null, // Will be set to a user ID
  },
  {
    content: "Mahadsanid! Tallaabooyinkan ayaa iga caawiyay inaan fahmo full-stack development.",
    author: null, // Will be set to a user ID
  },
  {
    content: "Flutter iyo React Native labadaba waa kuwo fiican, laakiin waxay ku xiran tahay baahiyaha project-ka.",
    author: null, // Will be set to a user ID
  },
  {
    content: "Database normalization waa muhiim, laakiin denormalization ayaa loo baahan yahay performance-ka dartii.",
    author: null, // Will be set to a user ID
  },
  {
    content: "CI/CD pipelines ayaa beddelaya habka aan ku sameyno deployment-ka.",
    author: null, // Will be set to a user ID
  },
  {
    content: "REST iyo GraphQL labadaba waxay leeyihiin faa'iidooyinkooda. Waxay ku xiran tahay use case-ka.",
    author: null, // Will be set to a user ID
  },
  {
    content: "Machine Learning waa mustaqbal. Waxaan ku faraxsanahay inaan barto!",
    author: null, // Will be set to a user ID
  },
  {
    content: "Security waa continuous process, mana aha hal tallaabo.",
    author: null, // Will be set to a user ID
  },
  {
    content: "Cloud computing ayaa fududeynaya IT infrastructure management.",
    author: null, // Will be set to a user ID
  },
];
...
- **Python**: Fudud oo loo isticmaalaa data analysis iyo AI
...
Programming-ka waa safar xiiso leh oo aan dhammaanayn. Ku dadaal oo ha quusan!`,
  ...
  },
  {
    ...
    content: `React Hooks waxay beddeleen habkii loo maamulo state-ka iyo lifecycle-ka ee React components-ka. Maqaalkaan waxaan ku baari doonaa hooks-ka ugu muhiimsan.
...
const seedBlogsWithComments = async () => {
  try {
    ...
    if (authors.length === 0 || users.length === 0) {
      console.error('No authors or users found. Please seed them first.');
      process.exit(1);
    }

    console.log(`Found ${authors.length} authors and ${users.length} users`);

    // Clear existing blogs
    await Blog.deleteMany({});
    console.log('Cleared existing blogs');

    // Prepare blogs with authors and comments
    const blogsWithData = blogs.map((blog, index) => {
      // Assign author (alternate between available authors)
      const authorId = authors[index % authors.length]._id;

      // Add 1-3 comments per blog
      const numComments = Math.floor(Math.random() * 3) + 1;
      const blogComments = [];

      for (let i = 0; i < numComments; i++) {
        const commentIndex = (index * 3 + i) % sampleComments.length;
        blogComments.push({
          ...sampleComments[commentIndex],
    ...
    // Insert blogs with embedded comments
    await Blog.insertMany(blogsWithData);
    console.log(`Successfully seeded ${blogs.length} blogs with embedded comments`);

    // Count total comments
    const totalComments = blogsWithData.reduce((sum, blog) => sum + blog.comments.length, 0);
    console.log(`Total comments created: ${totalComments}`);

    process.exit();
  } catch (error) {
    console.error('Error seeding blogs with comments:', error);
    process.exit(1);
  }
};

seedBlogsWithComments();
