...
const ManageOrders = () => {
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500 overflow-x-hidden max-w-full">
            <div>
                <h1 className="text-2xl font-bold text-gray-900 font-[Inter]">Order Invoices</h1>
                <p className="text-gray-500 text-sm mt-1">Track and manage all student enrolment payments.</p>
            </div>
            ...
            {loading ? (
                <div className="flex flex-col items-center justify-center py-20 gap-4">
                    ...
                    <p className="text-gray-500 font-bold animate-pulse">Dalabka waa la soo akhrinayaa...</p>
                </div>
                ...
                <div className="bg-white rounded-3xl border-2 border-dashed border-gray-100 py-20 text-center">
                    ...
                    <h3 className="text-gray-900 font-bold">Wax dalab ah lama helin</h3>
                    <p className="text-gray-400 text-sm mt-1">Isku day inaad wax kale raadiso ama bedesho filter-ka.</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 lg:grid-cols-2 2xl:grid-cols-3 gap-6">
                    {filteredOrders.map((order) => (
                        <div key={order._id} className="group relative bg-white rounded-[2.5rem] border border-gray-100 p-6 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-500 overflow-hidden flex flex-col">
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <div className="min-w-0">
                                        ...
                                        <div className="flex flex-col gap-0.5 mt-0.5">
                                            <p className="text-[11px] text-gray-500 font-medium truncate">{order.userDetails?.email}</p>
                                            <p className="text-[11px] text-emerald-600 font-bold">{order.phoneNumber}</p>
                                        </div>
                                    </div>
                                </div>
                                <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest ${order.status === 'active' ? 'bg-emerald-50 text-emerald-600 border border-emerald-100 shadow-sm' :
                                    order.status === 'pending' ? 'bg-amber-50 text-amber-600 border border-amber-100 shadow-sm' :
                            ...
                            <div className="bg-gray-50/80 rounded-3xl p-4 border border-gray-100 mb-6 flex-1">
                                <div className="flex gap-4 items-center mb-4">
                                    ...
                                    <div className="min-w-0">
                                        <span className="text-[9px] font-black text-emerald-600 uppercase tracking-widest block mb-0.5">Enrolled Course</span>
                                        <h5 className="text-sm font-bold text-gray-800 line-clamp-1" title={order.courseDetails?.title}>
                                ...
                                <div className="pt-4 border-t border-gray-200/50 grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <p className="text-[11px] font-black text-gray-700 uppercase tracking-tight">{order.paymentMethod}</p>
                                    </div>
                                    <div className="text-right">
                                        <span className="text-[9px] font-bold text-gray-400 uppercase tracking-widest block mb-1">Purchase Date</span>
                                        <p className="text-[11px] font-black text-gray-700">{new Date(order.createdAt).toLocaleDateString()}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center justify-between gap-4 pt-2">
                                <div>
                                    ...
                                    <div className="flex items-center gap-2">
                                        <span className="text-xl font-black text-gray-900 tracking-tighter">${order.finalPrice?.toFixed(2)}</span>
                                        {order.discountApplied > 0 && (
                                            <span className="text-[9px] text-emerald-600 font-extrabold bg-emerald-50 px-1.5 py-0.5 rounded border border-emerald-100">-${order.discountApplied}</span>
                                        )}
                                    </div>
                                </div>
            ...
            {selectedOrder && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
                    <div className="bg-white w-full max-w-2xl rounded-[2.5rem] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300 border border-gray-100">
                        {/* Modal Header */}
                        ...
                        <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                            ...
                            <div className="flex gap-6 items-start">
                                <div className="relative">
                                    ...
                                    <div className="absolute -bottom-2 -right-2 w-10 h-10 rounded-2xl border-2 border-white overflow-hidden shadow-lg bg-white">
                                ...
                                <div className="flex-1 pt-1">
                                    <h3 className="text-xl font-black text-gray-900 line-clamp-1">{selectedOrder.courseDetails?.title}</h3>
                                    <p className="text-sm text-gray-500 font-medium mt-1">Student: <span className="text-emerald-600 font-black">{selectedOrder.userDetails?.firstName} {selectedOrder.userDetails?.lastName}</span></p>
                                    <div className="flex items-center gap-3 mt-4">
                                        <div className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-wider ${selectedOrder.status === 'active' ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' :
                                            selectedOrder.status === 'pending' ? 'bg-amber-50 text-amber-600 border border-amber-100' :
                                                'bg-red-50 text-red-600 border border-red-100'
                                            }`}>
                                            {selectedOrder.status}
                                        </div>
                                        <span className="text-[10px] text-gray-400 font-bold border-l pl-3 border-gray-100 flex items-center gap-1">
                                    ...
                                    <div className="space-y-2.5">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="min-w-0 flex-1">
                                                ...
                                                <p className="text-xs font-bold text-gray-800 truncate">{selectedOrder.userDetails?.email}</p>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-3">
                                            <div className="w-8 h-8 bg-white rounded-xl flex items-center justify-center text-emerald-600 shadow-sm">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs font-bold text-gray-800">{selectedOrder.phoneNumber}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div className="space-y-2.5">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs font-black text-gray-800">${selectedOrder.finalPrice?.toFixed(2)}</p>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-3">
                                            <div className="w-8 h-8 bg-white rounded-xl flex items-center justify-center text-emerald-600 shadow-sm">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs font-bold text-gray-800 capitalize">{selectedOrder.paymentMethod}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
                                ...
                                <div className={`p-5 rounded-3xl border-2 border-dashed flex flex-col justify-center min-h-[160px] ${selectedOrder.paymentProof ? 'bg-white border-emerald-100' : 'bg-gray-50 border-gray-100'}`}>
                                    <h4 className={`text-[10px] font-black uppercase tracking-widest flex items-center gap-2 mb-3 ${selectedOrder.paymentProof ? 'text-emerald-600' : 'text-gray-400'}`}>
                                        <Eye size={12} /> Payment Proof
                                    </h4>
                                    {selectedOrder.paymentProof ? (
                                        <div className="relative group rounded-2xl overflow-hidden shadow-sm aspect-video bg-gray-50 border border-gray-100">
                                            <img
                                ...
                                <div className="p-6 bg-emerald-50/50 rounded-3xl border border-emerald-100/50 flex flex-col justify-between">
                                    <div>
                                        <div className="flex justify-between items-center mb-4">
                                            <span className="text-xs font-bold text-gray-700">Refrence</span>
                                            <span className="text-[10px] font-black text-emerald-700 uppercase">#{selectedOrder._id.slice(-8)}</span>
                                        </div>
                                        <div className="pt-4 border-t border-emerald-100">
                                            <span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">Enrollment</span>
                                    ...
                                    <div className="pt-4 mt-auto">
                                        <span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">Transaction Time</span>
                                        <p className="text-xs font-black text-gray-900 mt-0.5">{new Date(selectedOrder.createdAt).toLocaleTimeString()}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="p-6 bg-gray-50 border-t border-gray-100 flex justify-end gap-3">
                            <button
                                onClick={() => setSelectedOrder(null)}
                                className="px-6 py-2.5 bg-white hover:bg-gray-100 text-gray-600 rounded-2xl text-[11px] font-black uppercase tracking-widest transition-all border border-gray-200 active:scale-95"
                            >
                                Close View
                            </button>
                            {(selectedOrder.status === 'pending' || selectedOrder.status === 'rejected') && (
                                <button
                                    ...
                                    className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl text-[11px] font-black uppercase tracking-widest transition-all shadow-lg shadow-emerald-200 active:scale-95"
                                >
                                    Confirm Enrollment
                                </button>
                            )}
                ...
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
                    <div className="bg-white w-full max-w-md rounded-[2.5rem] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300 border border-gray-100">
                        <div className="p-8 text-center">
                            ...
                            <h3 className="text-xl font-black text-gray-900 mb-2 font-[Inter]">Ma hubtaa boss?</h3>
