...
import { Plus, Edit2, Trash2, Eye, Layout, CheckCircle, XCircle, Search, MoreVertical, AlertTriangle } from 'lucide-react';
...
const ManageCertificates = () => {
    ...
    return (
        <div className="p-4 md:p-8 min-h-screen bg-gray-50/50 dark:bg-slate-900 transition-colors duration-300">
            {/* Header Area */}
            <div className="max-w-7xl mx-auto space-y-8">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    ...
                    <button
                        onClick={() => navigate('/admin/certificates/builder')}
                        className="group flex items-center justify-center gap-3 px-8 py-4 bg-emerald-600 hover:bg-emerald-500 text-white rounded-2xl font-black text-sm shadow-xl shadow-emerald-500/20 hover:shadow-emerald-500/40 transition-all active:scale-95 overflow-hidden relative"
                    >
                ...
                {loading ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {filteredTemplates.map((template) => (
                            ...
                            >
                                {/* Preview Thumbnail */}
                                <div className="aspect-[1.414/1] bg-slate-100 dark:bg-slate-800 relative group-hover:scale-[1.02] transition-transform duration-700 overflow-hidden cursor-pointer"
                                    onClick={() => navigate(`/admin/certificates/builder?id=${template._id}`)}>
                                    <div
                                    ...
                                    <div className="absolute top-4 right-4">
                                        <div className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest backdrop-blur-md shadow-lg ${template.isActive ? 'bg-emerald-500 text-white' : 'bg-slate-500 text-white'}`}>
                                ...
                                <div className="p-6 space-y-4">
                                    <div className="flex justify-between items-start">
                                        ...
                                        <div className="flex gap-2">
                                            <button
                                                onClick={() => toggleStatus(template._id, template.isActive)}
                                                className={`p-3 rounded-xl transition-all ${template.isActive ? 'text-emerald-500 bg-emerald-50 dark:bg-emerald-500/10' : 'text-slate-400 bg-slate-50 dark:bg-slate-800'}`}
                                            ...
                                            <button
                                                ...
                                                className="p-3 bg-rose-50 dark:bg-rose-500/10 text-rose-500 rounded-xl hover:bg-rose-500 hover:text-white transition-all shadow-sm group/del"
                                                title="Delete"
                                            >
                                    ...
                                    <div className="pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-4">
                                        ...
                                        <button
                                            onClick={() => navigate(`/admin/certificates/builder?id=${template._id}`)}
                                            className="text-xs font-black text-emerald-600 hover:text-emerald-500 flex items-center gap-2 group/btn"
                                        >
                                            Full Edit
